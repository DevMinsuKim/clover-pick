...
import {
  LOTTO_RECORDS_PAGE_SIZE,
  type LottoRecordsInput,
  LottoRecordsInputSchema,
  type LottoRecordsKind,
  type LottoRecordsPage,
} from "./lottoRecordsContracts";

export async function getLottoRecords(
  kind: LottoRecordsKind,
  input: LottoRecordsInput = {},
): Promise<LottoRecordsPage> {
  const parsed = LottoRecordsInputSchema.parse(input);
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      const totalPages = Math.max(
        1,
        Math.ceil(totalCount / LOTTO_RECORDS_PAGE_SIZE),
      );
      const page = Math.min(parsed.page, totalPages);
      const window = {
        where,
        take: LOTTO_RECORDS_PAGE_SIZE,
        skip: (page - 1) * LOTTO_RECORDS_PAGE_SIZE,
      };
