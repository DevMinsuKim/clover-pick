...
If a set count is explicitly requested, preserve it exactly in requestedSets, even outside 1..5; otherwise null. Never clamp.
...
Return issue=none only when every requested condition can be represented.`,
  ...
  if (output.issue !== "none")
    throw new LottoInputError(
      "조건을 정확히 적용하기 어려워요. 포함·제외할 번호, 홀수·짝수 개수나 세트 수를 구체적으로 적어 주세요. 빠른 조건을 선택해도 좋아요.",
    );
  return output;
}
