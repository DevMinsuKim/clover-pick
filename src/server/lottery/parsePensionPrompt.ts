...
export const pensionExtractionSchema = z.strictObject({
  ...
  requestedCount: z
    ...
    .nullable()
    .describe(
      "생성할 복권 번호의 개수. 2개/2장/2매는 2. 모든 조 1세트는 5, 2세트는 10. 개수 언급이 없을 때만 null.",
    ),
...
export async function parsePensionPrompt(prompt: string) {
  ...
  const { output } = await generateText({
    ...
    instructions: `한국어 요청에서 연금복권720+의 생성 조건과 생성할 번호 개수를 빠짐없이 추출하세요.
"2개", "3개", "5개"는 만들어 줄 번호의 개수(requestedCount)입니다. 숫자 자릿수가 아닙니다. "앞 두 자리"는 prefix, "끝 두 자리"는 suffix의 길이이며 개수와 다릅니다.
숫자 0은 유효한 숫자이고 여섯 자리의 앞에 올 수 있습니다. "7을 넣어"는 숫자 7을 한 번 이상 포함하라는 뜻입니다. 특정 자리나 빈도를 임의로 요구하는 뜻이 아닙니다.
예시:
- "3조로, 끝 두 자리는 07로 고정해서 3개 만들어줘." => groups=[3], suffix="07", requestedCount=3, allGroups=false, issue="none".
...
"모든 조", "각 조", "같은 번호로 1~5조" means allGroups=true. Explicit single/group subset means allGroups=false. Otherwise allGroups=null. For allGroups=true, one set means FIVE tickets. Explicit two such sets => requestedCount=10, never clamp. For a count of tickets preserve it exactly even outside 1..5; otherwise requestedCount=null.
...
Unrelated requests => not_lottery; unclear meaning => ambiguous. issue=none only if ALL requested conditions are represented.`,
  ...
  if (output.issue !== "none")
    throw new LotteryInputError(
      "조건을 정확히 적용하기 어려워요. 원하는 조, 앞·끝자리, 포함·제외할 숫자나 생성 개수를 구체적으로 적어 주세요. 빠른 조건을 선택해도 좋아요.",
    );
  return output;
}
