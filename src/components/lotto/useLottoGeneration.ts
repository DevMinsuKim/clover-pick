...
export function useLottoGeneration(onBusyChange: (busy: boolean) => void) {
  const [result, setResult] = useState<LottoGeneration | null>(null);
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);
  const active = useRef(false);
  const pending = useRef<{ signature: string; requestId: string } | null>(null);

  async function generate(input: Omit<LottoCreateInput, "requestId">) {
    ...
    try {
      ...
      const response = await lottoCreateNumberActions({
        ...input,
      ...
      if (response.success) {
        ...
        void Promise.allSettled([
          client.invalidateQueries({ queryKey: ["lottoHistory"] }),
