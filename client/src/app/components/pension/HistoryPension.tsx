...
export default function HistoryPension() {
  ...
  const getRoundNumbers = async () => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/pension/9`
      );
          ...
          {roundNumber.map((row, rowIndex) => (
            <ul key={rowIndex}>
              <li className="flex flex-col relative bg-indigo-600 rounded-3xl">
                  ...
                  {row.number.map((number, columnIndex) => (
                    <React.Fragment key={columnIndex}>
                      <div
                        ...
                        } ${IndexBgSelect(columnIndex)} my-1`}
                      >
                        {number}
                      </div>
                      <div
                        ...
                        } flex rounded-full items-center self-center justify-center w-8 h-8 md:w-9 md:h-9 xl:w-8 xl:h-8 3xl:w-10 3xl:h-10 
                     font-bold text-xl bg-white text-indigo-600`}
                      >
                        조
                      </div>
                    </React.Fragment>
                ...
                <div className="flex mx-3 mb-2 justify-between flex-wrap">
                  {row.bonusNumber.map((number, columnIndex) => (
                    <React.Fragment key={columnIndex}>
                      <div
                        className={`${
                          columnIndex !== 0 && "hidden"
